...
SKILL_PATH      = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "skills", "SKILL.md")
BASE_SKILL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "skills", "SKILL_BASE.md")
...
"forbidden_names": list of exact report/dashboard/filter names from the BRD that should
  ...
  "Top10SaleItemsReport", "RegionFilter", "DateRangePrompt"]) and subject area names as forbidden names.
"subject_area": one sentence describing what this BRD covers. This is just for context and SKILL.md labeling (e.g. "This BRD covers sales and profit analysis across different regions and time periods.")
"Entities": list of key entities mentioned in the BRD (e.g. ["Sales", "Profit", "Region", "Time"])
"Dimension Entities": Dimensions entities provide the “who, what, where, when, why, and how” context surrounding a business process event. Dimension tables contain the descriptive attributes used by BI applications for filtering and grouping the facts.Find list of key dimensions mentioned in the BRD (e.g. ["Geography", "Product", "Time"])
"Fact Entities": Fact entities represent the core business process events or transactions that the organization wants to analyze. They typically contain numeric measures and foreign keys to dimension entities. Find list of key facts mentioned in the BRD (e.g. ["Sales", "Profit"])
